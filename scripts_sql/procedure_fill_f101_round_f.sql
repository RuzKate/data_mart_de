...
begin
    ...
    with wt_f101_round as
    (select  date_trunc('month', i_OnDate)		as from_date,
           (date_trunc('MONTH', i_OnDate) 
           + INTERVAL '1 MONTH - 1 day')  		as to_date,
    ...
    select *,
    	   wt.balance_out_rub + wt.balance_out_val	as balance_out_total
      from (select *,
      			   case
	      			 when r.characteristic = 'A'
                 	 then r.balance_in_rub - r.turn_cre_rub + r.turn_deb_rub
                 	 
                 	 when r.characteristic = 'P'
                 	 then r.balance_in_rub + r.turn_cre_rub - r.turn_deb_rub
                 	 else 0
               	   end								as balance_out_rub,
               	   case
	      			 when r.characteristic = 'A'
                 	 then r.balance_in_val - r.turn_cre_val + r.turn_deb_val
                 	 
                 	 when r.characteristic = 'P'
                 	 then r.balance_in_val + r.turn_cre_val - r.turn_deb_val
                 	 else 0
               	   end								as balance_out_val
      		  from wt_f101_round r
             ) as wt;
